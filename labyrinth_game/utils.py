from labyrinth_game.constants import COLORS, COMMANDS, ROOMS
...
def describe_current_room(game_state: dict):
    ...
    if game_state is None:
        return
    print(
        f"{COLORS['BLUE']}== {game_state['current_room'].upper()} =={COLORS['RESET']}"
    )
    ...
    if ROOMS[game_state["current_room"]]["puzzle"] is not None:
        print(
            f"{COLORS['BLUE']}Кажется, здесь есть загадка \
(используйте команду solve).{COLORS['RESET']}"
        )
...
def solve_puzzle(game_state: dict):
    ...
    if ROOMS[game_state["current_room"]]["puzzle"] is not None:
        ...
        if answer == "quit":
            ...
            answer == ROOMS[game_state["current_room"]]["puzzle"][1]
            or answer == "десять"
            and game_state["current_room"] == "hall"
        ):
            print(
                f"Отличное решение! Ваша награда:\
{COLORS['YELLOW']}{ROOMS[game_state['current_room']]['items']}{COLORS['RESET']}"
            )
            for item in ROOMS[game_state["current_room"]]["items"]:
    ...
    if "treasure_key" in game_state["player_inventory"]:
        ...
        if answer == "quit":
            ...
            if answer == ROOMS[game_state["current_room"]]["puzzle"][1]:
                ...
                ROOMS[game_state["current_room"]]["items"].remove("treasure_chest")
                print(f"{COLORS['BOLD']}В сундуке сокровище!\
Вы победили!{COLORS['RESET']}")
                game_state["game_over"] = True
            else:
                print("Неверно. Попробуйте снова.")
        else:
...
def pseudo_random(seed, modulo: int) -> int:
    """Генерация случайного числа от 0 до n, n - не включительно.
    Принимает сид и число n."""
...
def trigger_trap(game_state: dict):
    ...
    if len(game_state["player_inventory"]) > 0:
        game_state["player_inventory"].pop(
            pseudo_random(
        ...
        if (
            pseudo_random(game_state["steps_taken"], EVENT_PROBABILITY)
            < LUCKY_THRESHOLD
        ):
            print(
                f"{COLORS['RED']}Вы упали в яму с шипами!\
Игра окончена!{COLORS['RESET']}"
            )
            game_state["game_over"] = True
        else:
